...
Preprocessing_FERS_new<- function(Preproc){
  ...
  # for i in species
  if(is_empty(Preproc$coeff_a)){
    Preproc$coeff_a<-NA}
  if(is_empty(Preproc$coeff_b)){
    Preproc$coeff_b<-NA
  }
  
  if(any(is.na(Preproc$coeff_a))){
    coeffs<- list()  
    coeffs_plot<-list()
    coeffs_plot<-data.frame(Cohort_name=NA,idplot=NA,coeff_a=NA,coeff_b=NA)
    n<-1
    s<-1
    for ( i in unique(Preproc$cohort_name)){
      if (NROW(Preproc[Preproc$cohort_name==i & !is.na(Preproc$Height_m),])>=5){
        ...
        coeffs[[n]]<-data.frame(Cohort_name=i,coeff_a=summary(coeeffs_model)$coef["a","Estimate"],coeff_b=summary(coeeffs_model)$coef["b","Estimate"])  #store parameters
        n<-n+1
      } else{
        coeffs[[n]]<-data.frame(Cohort_name=i,coeff_a=NA,coeff_b=NA)  #store parameters
        n<-n+1
      }
      for(idp in unique(Preproc$idplots)){ # for i in id plots
        if (NROW(Preproc[Preproc$cohort_name==i & !is.na(Preproc$Height_m) & Preproc$idplots == idp ,])>5){
          ...
          coeffs_plot[s,]<-data.frame(Cohort_name=i,idplot=idp,coeff_a=summary(coeeffs_model_plot)$coef["a","Estimate"],coeff_b=summary(coeeffs_model_plot)$coef["b","Estimate"])  #store parameters
          s<-s+1
          ...
          coeffs_plot[s,"coeff_a"]<-  coeffs[sapply(coeffs, "[[", "Cohort_name") == i][[1]]$coeff_a #store parameters
          coeffs_plot[s,"coeff_b"]<-  coeffs[sapply(coeffs, "[[", "Cohort_name") == i][[1]]$coeff_b #store parameters
    ...
    for ( i in unique(Preproc$cohort_name)){
      for ( idp in unique(Preproc$idplots)){
        if(!is_empty(Preproc[Preproc$cohort_name==i & Preproc$idplots == idp,]$coeff_a)){
          if(!is.na(coeffs_plot[coeffs_plot$Cohort_name==i & coeffs_plot$idplot == idp,"coeff_a"])){
            Preproc[Preproc$cohort_name==i & Preproc$idplots == idp,]$coeff_a<-coeffs_plot[coeffs_plot$Cohort_name==i & coeffs_plot$idplot == idp,"coeff_a"]
            Preproc[Preproc$cohort_name==i & Preproc$idplots == idp,]$coeff_b<-coeffs_plot[coeffs_plot$Cohort_name==i & coeffs_plot$idplot == idp,"coeff_b"]
          }
        }
      }
    }
  ...
  Preproc$Estim_Ht<-1.3+exp(Preproc$coeff_a+Preproc$coeff_b/Preproc$dbhcm)
  ...
  Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Spruce",]$Estim_Ht<- (112.614046-0.341039*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Spruce",]$plotba_m2_ha +
                                                                                  0.008297*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Spruce",]$Density_ha + 
                                                                                  0.550804*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Spruce",]$BAL)* 
    (1-exp(-0.013665*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Spruce",]$dbhcm^(0.794101))) 
  
  
  #Larch & Pine
  # (31.482382+0.255232*plotba_m2_ha-0.021031*Density_ha)*(1-exp(-0.029592*dbhcm^(1.035300+0.003920*BAL_ties_adjusted_3)))
  
  Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Pine",]$Estim_Ht <- (16.9015+0.08348*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Pine",]$BAL_ties_adjusted_3+
                                                                                 0.08036*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Pine",]$plotba_m2_ha)*
    (1-exp(-0.04292*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Pine",]$dbhcm))
  
  Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Larch",]$Estim_Ht <- (32.5922+0.1052*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Larch",]$BAL_ties_adjusted_3 + 
                                                                                  0.1229*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Larch",]$plotba_m2_ha)*
    (1-exp(-0.0235*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Larch",]$dbhcm))
  # Douglas fir
  ...
  Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Douglas fir",]$Estim_Ht<- (78.755716-0.384251*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Douglas fir",]$plotba_m2_ha+
                                                                                       0.011092*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Douglas fir",]$Density_ha-
                                                                                       0.625095*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Douglas fir",]$BAL)*
    (1-exp(-0.009568*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Douglas fir",]$dbhcm^(1.024247+
                                                                                                     0.007481*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Douglas fir",]$BAL))) 
  # Other conifers
  # Height_m~(131.068583-3.068923*plotba_m2_ha+0.060772*Density_ha)*(1-exp(-0.003678*dbhcm^(1.291154+0.005438*BAL_ties_adjusted_3)))
  Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Other conifers" ,]$Estim_Ht<- (77.995880-1.097861*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Other conifers" ,]$plotba_m2_ha+
                                                                                           0.020173*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Other conifers" ,]$Density_ha-
                                                                                           0.642245*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Other conifers" ,]$BAL)*(1-exp(-
                                                                                                                                                                                             0.003564*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Other conifers" ,]$dbhcm^
                                                                                                                                                                                             (1.325656+0.014722*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Other conifers" ,]$BAL))) 
  # "Slow-growing broadleaves"
  #Height_m~(26.640039-0.004558*Density_ha)*(1-exp(-0.017944*dbhcm^(1.302086)))
  Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Slow-growing broadleaves" ,]$Estim_Ht<-(29.6779+0.1034*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Slow-growing broadleaves" ,]$BAL )*
    (1-exp(-0.04494*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Slow-growing broadleaves" ,]$dbhcm^0.7813)) 
  # "Fast-growing broadleaves"
  #(24.425932+0.029322*plotba_m2_ha)*(1-exp(-0.011050*dbhcm^(1.669975)))
  Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Fast-growing broadleaves" ,]$Estim_Ht<-(14.6661+0.1167*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Fast-growing broadleaves" ,]$BAL+
                                                                                                    0.01872*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Fast-growing broadleaves" ,]$plotba_m2_ha)*
    (1-exp(-0.0761624*Preproc[is.na(Preproc$Estim_Ht) & Preproc$cohort_name=="Fast-growing broadleaves" ,]$dbhcm)) 
  ...
  for (i in Preproc$Xi) {
    ...
    Preproc[Preproc$cohort_name=="Spruce" & Preproc$Xi==i,]$Logit_CR <- -5.871e-01-
      4.598e-03*Preproc[Preproc$cohort_name=="Spruce" & Preproc$Xi==i,]$BAL+
      2.426e-01*log(Preproc[Preproc$cohort_name=="Spruce" & Preproc$Xi==i,]$CCF) -
      2.721e-02*Preproc[Preproc$cohort_name=="Spruce" & Preproc$Xi==i,]$Height_imp -
      7.982e-01*(Preproc[Preproc$cohort_name=="Spruce" & Preproc$Xi==i,]$Height_imp/Preproc[Preproc$cohort_name=="Spruce" & Preproc$Xi==i,]$dbhcm) + 
      1.543e-04*Preproc[Preproc$cohort_name=="Spruce" & Preproc$Xi==i,]$dbhcm^2 
    #Pine 
    ...
    Preproc[Preproc$cohort_name=="Slow-growing broadleaves" & Preproc$Xi==i,]$Logit_CR <--1.38486-
      0.02662* Preproc[Preproc$cohort_name=="Slow-growing broadleaves" & Preproc$Xi==i,]$BAL+ 
      0.01441* Preproc[Preproc$cohort_name=="Slow-growing broadleaves" & Preproc$Xi==i,]$plotba_m2_ha+
      1.20780*( Preproc[Preproc$cohort_name=="Slow-growing broadleaves" & Preproc$Xi==i,]$Height_imp/ Preproc[Preproc$cohort_name=="Slow-growing broadleaves" & Preproc$Xi==i,]$dbhcm) 
    
    #fast growing Broadleave
    #=2.4539-0.00941*R33-0.1457*LN(Z33)-0.04574*AC33-0.5915*(AC33/I33)+0.000162*I33^2
    Preproc[Preproc$cohort_name=="Fast-growing broadleaves" & Preproc$Xi==i,]$Logit_CR <- 7.2101096-
      0.0550380*Preproc[Preproc$cohort_name=="Fast-growing broadleaves" & Preproc$Xi==i,]$BAL-
      1.4634666*log(Preproc[Preproc$cohort_name=="Fast-growing broadleaves" & Preproc$Xi==i,]$CCF)+
      0.0580135*Preproc[Preproc$cohort_name=="Fast-growing broadleaves" & Preproc$Xi==i,]$plotba_m2_ha -
      1.1701386*(Preproc[Preproc$cohort_name=="Fast-growing broadleaves" & Preproc$Xi==i,]$Height_imp/Preproc[Preproc$cohort_name=="Fast-growing broadleaves" & Preproc$Xi==i,]$dbhcm) -
      0.0003753*Preproc[Preproc$cohort_name=="Fast-growing broadleaves" & Preproc$Xi==i,]$dbhcm^2 
    # Larch
    # =5.8306-0.01822*R136-0.7948*LN(Z136)-0.03937*AC136
    Preproc[Preproc$cohort_name=="Larch" & Preproc$Xi==i,]$Logit_CR <- 1.6527801 - 
      0.0627860*Preproc[Preproc$cohort_name=="Larch" & Preproc$Xi==i,]$BAL -
      0.0003818*Preproc[Preproc$cohort_name=="Larch" & Preproc$Xi==i,]$dbhcm^2
    #Other Conifers
    # =4.1759-0.01941*R137-0.3945*LN(Z137)-0.0965*AC137+0.000463*I137^2
    Preproc[Preproc$cohort_name=="Other conifers" & Preproc$Xi==i,]$Logit_CR <- 1.33314 + 
      0.07584*Preproc[Preproc$cohort_name=="Other conifers" & Preproc$Xi==i,]$BAL-
      3.09514*(Preproc[Preproc$cohort_name=="Other conifers" & Preproc$Xi==i,]$Height_imp/Preproc[Preproc$cohort_name=="Other conifers" & Preproc$Xi==i,]$dbhcm) 
    
    # Douglas Fir
    Preproc[Preproc$cohort_name=="Douglas fir" & Preproc$Xi==i,]$Logit_CR <- 1.914e-01-1.425e-02*Preproc[Preproc$cohort_name=="Douglas fir" & Preproc$Xi==i,]$BAL+ 
  ...
  #################  #################  #################  #################  #################
  #################  #################  #################  #################
  # new dbh increment models
  Preproc$DBH_Increment_NEW<-NA
  #Spruce
  Preproc[Preproc$cohort_name=="Spruce",]$DBH_Increment_NEW <-  -2.621+
    Preproc[Preproc$cohort_name=="Spruce",]$YC/16.75991*
    (1.087e-04*Preproc[Preproc$cohort_name=="Spruce",]$dbhcm^2-
       3.656e-03*Preproc[Preproc$cohort_name=="Spruce",]$BAL_ties_adjusted_3+
       Preproc[Preproc$cohort_name=="Spruce",]$CCF^-4.398e-01)+
    exp((Preproc[Preproc$cohort_name=="Spruce",]$Height_imp)^3.871e-02)
  
  Preproc[Preproc$cohort_name=="Douglas fir",]$DBH_Increment_NEW <--2.191+
    Preproc[Preproc$cohort_name=="Douglas fir",]$YC/26*
    (4.421e-05*Preproc[Preproc$cohort_name=="Douglas fir",]$dbhcm^2-
       4.831e-03*Preproc[Preproc$cohort_name=="Douglas fir",]$BAL_ties_adjusted_3)+
    exp((Preproc[Preproc$cohort_name=="Douglas fir",]$Height_imp/
           Preproc[Preproc$cohort_name=="Douglas fir",]$dbhcm)^-1.096e-01)
  
  
  
  Preproc[Preproc$cohort_name=="Fast-growing broadleaves",]$DBH_Increment_NEW <- -6.9983079+
    exp((Preproc[Preproc$cohort_name=="Fast-growing broadleaves",]$Height_imp)^0.0220661+
          -0.0008160*Preproc[Preproc$cohort_name=="Fast-growing broadleaves",]$BAL_ties_adjusted_3+
          -0.0002173*Preproc[Preproc$cohort_name=="Fast-growing broadleaves",]$dbhcm+
          Preproc[Preproc$cohort_name=="Fast-growing broadleaves",]$CCF^-0.0080254)
  
  Preproc[Preproc$cohort_name=="Slow-growing broadleaves",]$DBH_Increment_NEW<- -6.8532705+
    exp((Preproc[Preproc$cohort_name=="Slow-growing broadleaves",]$Height_imp)^0.0159007+
          -0.0006533*Preproc[Preproc$cohort_name=="Slow-growing broadleaves",]$BAL_ties_adjusted_3+
          -0.0001409*Preproc[Preproc$cohort_name=="Slow-growing broadleaves",]$dbhcm+
          Preproc[Preproc$cohort_name=="Slow-growing broadleaves",]$CCF^-0.0095174)
